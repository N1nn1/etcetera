...
public class HandbellItem extends Item {
    ...
    public HandbellItem(Properties properties) {
        super(properties);
    }
    ...
    private static void applyParticlesTooFriends(Level world, BlockPos pos, List<LivingEntity> hearingEntities, Player player) {
        ...
        hearingEntities.stream().filter(entity -> isFriendEntity(pos, entity, player)).forEach(entity -> {
            ...
            for (int k = 0; k < j; ++k) {
                ...
                float h = (float) FastColor.ARGB32.red(l) / 255.0F;
                float m = (float) FastColor.ARGB32.green(l) / 255.0F;
                float n = (float) FastColor.ARGB32.blue(l) / 255.0F;

                world.addParticle(ColorParticleOption.create(net.minecraft.core.particles.ParticleTypes.ENTITY_EFFECT, h, m, n), player.getX(), player.getY() + 1, player.getZ(), 0, 0, 0);
            }
        });
    }
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level world, Player player, @NotNull InteractionHand hand) {
        ...
        world.playSound(player, player.blockPosition(), EtceteraSoundEvents.ITEM_HANDBELL_RING, SoundSource.PLAYERS, 0.3f, 1.0f);
