...
public class TurtleRaftRenderer extends EntityRenderer<TurtleRaftEntity> {
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/entity/boat/turtle_raft_overlay.png");
    ...
    private TurtleRaftColorRenderLayer createLayer(EntityRendererProvider.Context ctx) {
        return new TurtleRaftColorRenderLayer(new RenderLayerParent<>() {
            @Override
            public @NotNull TurtleRaftModel getModel() {
                return TurtleRaftRenderer.this.model;
            }

            @Override
            public @NotNull ResourceLocation getTextureLocation(@NotNull TurtleRaftEntity entity) {
                return TEXTURE;
            }
        }, ctx.getModelSet());
    }
    ...
    @Override
    public void render(TurtleRaftEntity raft, float f, float g, PoseStack matrixStack, @NotNull MultiBufferSource vertexConsumerProvider, int i) {
        ...
        model.renderToBuffer(matrixStack, vertexConsumer, i, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
